//
//  IsolationExercisesListView.swift
//  Gym Help
//
//  Created by Павло on 22.04.2023.
//

import SwiftUI

struct IsolationExercisesListView: View {
        ...
        if searchText.isEmpty{
            if chosenMuscle == .none {
                return exercises.filter { exercise in
                    return exercise.mechanic == .isolation
                }
            }else{
                return exercises.filter { exercise in
                    return exercise.primaryMuscles.first == chosenMuscle && exercise.mechanic == .isolation
                }
            }
        }else{
            return exercises.filter { exercise in
               return exercise.name.contains(searchText) && exercise.mechanic == .isolation
            }
        }
    }
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                ...
                .navigationTitle("Isolation Exercises")
...
struct IsolationExercisesListView_Previews: PreviewProvider {
